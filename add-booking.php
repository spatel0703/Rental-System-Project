...
if(isset($_SESSION['user_id']) && isset($_SESSION['user_email'])){
    ...
    include "db_conn.php";
     ...
     $allPropIDS = get_property_ids($conn);

     // Filter property IDs to show only those owned by the current user
     $propIDS = array_filter($allPropIDS, function ($property) use ($conn) {
        $sql = "SELECT ownerId FROM rentals WHERE propertyID = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$property['propertyID']]);
        $owner = $stmt->fetch();

        return $owner && $owner['ownerId'] == $_SESSION['user_id'];
     });
    ...
    <form action="php/add-booking.php"
        ...
        <div class="mb-3">
            ...
            <select name="renter_ID" class="form-control">
                <?php foreach ($renterIDS as $renter) : ?>
                    <option value="<?= htmlspecialchars($renter['userID']); ?>">
                        <?= htmlspecialchars($renter['userID']); ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
